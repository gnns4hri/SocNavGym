...
def make_env():
    ...
    # Strip entity types whose MAX_* == 0 to avoid zero-shaped obs keys,
...
policy_kwargs = dict(
    features_extractor_class=GATv2Extractor,
    features_extractor_kwargs=dict(
        hidden_dim=gnn_cfg.get("hidden_dim", 128),
        n_heads=gnn_cfg.get("n_heads", 4),
        n_layers=gnn_cfg.get("n_layers", 2),
        features_dim=gnn_cfg.get("features_dim", 256),
    ),
)

model = SAC(policy="MultiInputPolicy", env=train_env, policy_kwargs=policy_kwargs, verbose=config["verbose"], tensorboard_log=f"runs/{run.id}", **sac_params)
